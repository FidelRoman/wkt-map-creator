...
const Map = dynamic(() => import('@/components/Map'), {
    ...
    loading: () => (
        <div className="flex flex-col items-center justify-center h-screen w-full bg-slate-50">
            ...
            <p className="text-slate-500 font-medium text-sm animate-pulse">Loading map…</p>
        </div>
    )
});
...
export default function EmbedViewer({ project, hideWatermark = false }: EmbedViewerProps) {
    ...
    return (
        <div className="relative w-full h-screen overflow-hidden">
            {/* isReadOnly={true} prevents draw controls from rendering in the Map/ActiveLayerEditor */}
            <Map
                layers={layers}
                activeLayerId={null}
                onUpdateLayer={() => {}}
            ...
            {!hideWatermark && (
                <a
                    ...
                    className="absolute bottom-2 right-2 z-[9999]"
                >
